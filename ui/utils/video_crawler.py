...
class VideoCrawlerForYHDMDM:
    ...
    def get_video_name(self, select_path=None):
        ...
        if is_state:
            ...
            self.absolute_path = self.create_folder(name=self.video_name, select_path=select_path)
            return True
        return False
    ...
    def get_m3u8_links(self, m3u8_file_path):
        # check it
        if os.path.exists(m3u8_file_path):
            with open(m3u8_file_path, 'r', encoding='utf-8') as file:
                content = file.readlines()
            # optimize
            cleaned_content = [item.strip() for item in content if item.strip()]
            if not any(line.strip() for line in cleaned_content):
                return False
            else:
                self.Get_alldata()
    ...
    def PreWork(self, select_path):
        # .json exist not enter this part
        is_existed = self.get_video_name(select_path=select_path)
        json_file_path = os.path.join(self.absolute_path, r'json\\shared-link-for-network.json')
...
#     video_crawler = VideoCrawlerForYHDMDM(target=target)
#     video_crawler.PreWork()
#     # check the m3u8 file has value
